...
- 지도자·미지정·투수·포수·내야수·외야수 통계 셀을 토글 버튼으로 만들고, 해당 범위 목록을 10행 단위 가로 스와이프 페이지로 표시한다. 통계 목록은 학교 카드 영역을 교체하며, 같은 항목을 다시 누르면 학교 카드가 복원된다.
...
- 투수·지도자·선수 검색 목록은 페이지당 10행이다. 통계 활성 시 학교 카드가 사라지고 비활성 시 복원됨을 확인했으며, 백넘버 `1` 검색 98명은 10페이지로 분할됐다.
