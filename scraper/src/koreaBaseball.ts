...
export function classifyAtBat(
  raw: string
): { ab: number; h: number; hr: number; bb: number; so: number } | null {
  const c = raw.replace(/\s/g, "");
  if (!c || c === "승부주자") return null;
  if (/(^|,)(4구|고4|고의4구)/.test(c)) return { ab: 0, h: 0, hr: 0, bb: 1, so: 0 }; // 볼넷
  if (/사구/.test(c)) return { ab: 0, h: 0, hr: 0, bb: 1, so: 0 }; // 몸에맞는공(출루)
  ...
  return { ab: isSac ? 0 : 1, h: isHit ? 1 : 0, hr: isHR ? 1 : 0, bb: 0, so: isK ? 1 : 0 };
}
...
function parseBatters(
  ...
  for (const row of rows) {
    ...
    const line: BatterLine = {
      ...
      b2: 0, b3: 0, hr: 0, bb: 0, so: 0, sb: 0,
    };
    cells.forEach((cell, i) => {
      const res = classifyAtBat(cell);
      if (!res) return;
      // 홈런/볼넷/삼진/도루는 타석 결과 코드에서 집계(합계 셀에 없음)
      line.hr += res.hr; line.bb += res.bb; line.so += res.so;
