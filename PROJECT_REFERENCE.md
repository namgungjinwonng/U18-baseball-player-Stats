...
- 2026-07-12: **1.5초 시작·새로고침 모션 분리**: 일반 앱 실행은 `launch-home-run.webm` 홈런 아크 1.5초로 고정. 상단 버튼·pull-to-refresh 수동 갱신은 재로드 직전 `refreshMotion.ts`가 홈런 아크/패스트 피치/스코어보드 플립 1.5초 중 하나를 균등 무작위로 `sessionStorage`에 저장한다. 새 문서의 `MobileLaunchMotion`이 이를 한 번 소비·삭제해 선택된 1개만 재생하며, 저장값이 없거나 잘못되면 홈런 아크로 폴백한다. 백그라운드 복귀 자동 갱신에는 모션을 재생하지 않는다.
