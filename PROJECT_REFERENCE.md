...
- 2026-07-11: **모바일 시즌리더 = 가로 스와이프 캐러셀 (전 항목 TOP10)**: `MHome` 시즌리더를 세로 카드 스택(9항목×9명)에서 캐러셀로 교체 — `.m-lead-track`(CSS scroll-snap x mandatory) + 슬라이드당 한 항목 TOP10, 상단 `.m-lead-chips` 항목 칩 바(활성 칩 자동 센터링, 탭 = 즉시 점프 — smooth 스크롤은 snap mandatory 와 충돌해 중간에 끊기므로 미사용) + "N/20" 힌트. `leaders.leaderboards()` 에 `categoryIds` 파라미터 추가(기본 HOME_CATEGORY_IDS 라 데스크탑 홈은 종전 9항목 유지, 모바일만 `CATEGORIES` 전 항목 20개). 순위 번호 `.rank-no` 표시. 필터 변경 시 첫 슬라이드로 복귀.
