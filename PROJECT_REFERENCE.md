...
- 2026-07-13: **다년 시즌(2024·2025) + 통산 모드 + 리그 평균 오버레이** (상세 = WORK_HISTORY.md): ① 과거 시즌 1회 수집 — scraper `YEAR` 환경변수 historical 모드(요청 간격/동시수 env 제어, 공식기록 50명 체크포인트 재개), `data/2024/`·`data/2025/`를 2026 동일 스키마로 생성, `years.json=[2026,2025,2024]`, 신규 `validateData.ts`(`npm run validate-data`)·`syncCareerIndex.ts`(`npm run career-index`). ② 데이터 재구성 — `matchups/{id}.json` 폴백 제거하고 선수 JSON `matchups` 필드에 병합(파일 수 연도당 ~2,400↓, 선수 상세 요청 1회화; by-tournament matchups 는 유지), `profiles/{personNo}.json` 에 `careerYears:{연도:선수ID}` 인라인(career-index.json 은 생성·검증용, 런타임 fetch 없음), `roster-history.json` 연도 폴더로 이동. ③ 선수 상세 연도 전환 — personNo 로 동일인 식별, 연도 변경 시 그 연도 `/player/{id}` 로 이동(무기록이면 `/person/{personNo}`, 복귀 자동), 연도 종속 `?t` 쿼리는 제거·나머지 유지, `p.id===id` 가드로 비동기 잔상 내비 방지. ④ 통산 모드 — 선수 상세 `{연도} 시즌 | 통산` 토글(`CareerPanel`·`career.ts`), 카운팅=합산·비율=원자료 재계산·wOBA=고정 가중치 재계산·wRC+=시즌값 PA 가중평균·WAR=시즌 합(각 시즌은 해당 연도 `averages.json` 기준, FIP 는 연도 상수 문제로 통산 제외), averages 결손 연도는 리그 대비 계산 제외. ⑤ 연도별 미니 바 차트 — 비율 지표에 그 해 리그 평균 빨간 세로 마커+`평균 값` 텍스트(값 칸 2줄), 스케일=선수값+평균값 전체 최대, 0 이하 값 막대 없음(음수 WAR), ERA/WHIP `낮을수록 좋음` 유지. ⑥ 연도 선택 = sessionStorage(`u18-selected-year`) — 리로드 유지+새 세션은 현재 연도 우선. ⑦ 워딩 — "전체 시즌"→"대회 전체", 경기일정 탭 "시합"→"대회", SaberTerm "시합별"→"대회별", weights "시합"→"대회"(UI 문자열만). ⑧ `sw.js` 캐시 v3→v4(데이터 재구성 후 구 캐시 정리). ⑨ 검증 스크립트 `verify-dev.ps1`(scraper tsc/테스트/validate-data + career 테스트 + web build).
- 2026-07-12: **랭킹 항목 = 드롭다운 + 모바일 상단 컨트롤 고정**: `LeadersView` 의 항목 칩 그리드(`.leader-cat-grid`)를 `타자/투수 토글 + 항목 `<select>`(`.leader-select`)로 교체(한 화면에 선수 더 많이). 모바일(`.m-page`)에선 `.leader-select`·`.m-records-head`(MRecords 탭)를 `position: sticky; top:56px`(topbar 아래)로 고정 → 선수 목록만 롤로 스크롤. 드롭다운은 데스크탑에도 적용(공용), sticky 는 모바일만. ⚠ 선수 기록 표는 `.stat-table__scroll` 가 가로 스크롤(overflow-x:auto → 스크롤 컨테이너)이라 페이지 기준 sticky thead 불가 — 탭만 고정하고 열 헤더는 스크롤됨(열 헤더 고정 원하면 표를 내부 스크롤 박스로).

- 2026-07-12: **서스펜디드(일시중단→재개) 경기 이중계상 제거**: KBSA 는 중단 시점 부분 경기와 재개 완료 경기를 다른 game_idx 로 게시하고, 재개 후 캘린더에서 부분 경기는 사라지지만 `data/games/{id}.json` 은 삭제되지 않아 남는다(경기수·기록·경기로그 이중계상). `accumulate.dropSupersededGames` 를 `readGames` 에 추가 — 같은 팀쌍·대회에서 이른 날짜 경기의 타자·투수가 이후 경기의 **부분집합이고 타자 수가 더 적으면** 그 경기를 집계 제외. 실측: 2026 시즌 orphan 2건(38853 세광고v배명고 7/9→7/10, 38751 밀양BCv야탑고 7/1→7/2)만 정확히 제외(같은 팀쌍+대회 2경기↑ 그룹 16개 중 오탐 0). 공식기록(official.json)의 `g` 는 KBSA 권위값이라 무관, box-score 파생(경기로그/시합별/무기록 선수 경기수)만 교정. **파일은 멱등/재수집 판단용으로 보존**(canceled 마커와 동일 원칙). 회귀 테스트 `runSuspendedGame` 추가. 배포 데이터 반영은 다음 scrape 재집계에서.
- 2026-07-12: **모바일 당겨서 새로고침 — 모달 열림 중 비활성**: `PullToRefresh.onStart` 에서 `.modal-backdrop` 존재 시 제스처 시작 안 함(모달 내부 스크롤이 새로고침으로 오인되던 문제). + 새로고침 즉시 모션(모래시계 대기 제거, `manualRefreshAndReload` 선조회·450ms 삭제).
- 2026-07-11: **모바일 시즌리더 = 가로 스와이프 캐러셀 (타자/투수 × 누적 TOP10)**: `MHome` 시즌리더를 세로 카드 스택(9항목×9명)에서 캐러셀로 교체 — `.m-lead-track`(CSS scroll-snap x mandatory) + 슬라이드당 한 항목 TOP10 + "N/M" 힌트 + 순위 번호 `.rank-no`. 상단 `.m-lead-chips` 는 **타자/투수 그룹 토글 2개**(항목 이동은 스와이프 전용). 항목 = 누적 위주: 타자 `avg,hr,rbi,h,r,sb`(6) / 투수 `era,so,w,sv,ip`(5) — 타율·평균자책 외 세이버(출루율/OPS/wOBA/wRC+/WAR/WHIP)는 제외(랭킹 페이지에서). `leaders.leaderboards()` 에 `categoryIds` 파라미터 추가(기본 HOME_CATEGORY_IDS 라 데스크탑 홈은 종전 9항목 유지). 그룹 전환·필터 변경 시 첫 슬라이드로 복귀. ⚠ snap mandatory 컨테이너에 `scrollTo({behavior:'smooth'})`는 중간 스냅에서 끊김(실측) — 프로그램 이동은 즉시 점프로.
- 2026-07-11: **PWA 스플래시 배경 화이트**: manifest `background_color` #111111→#ffffff — OS 스플래시(흰 배경+아이콘)가 흰색 런치 오버레이(홈런 아크)로 끊김 없이 이어짐. 하단 앱 이름 텍스트는 플랫폼 제어라 제거 불가(Android 12+는 시스템 스플래시가 아이콘만 표시). 설치된 WebAPK 는 manifest 재확인 주기에 따라 수일 내 자동 반영.
- 2026-07-11: **u81 동기화 종료 게이트**: scrape/teams/scrape-full 선행 잡에 `sunset` 출력(KST 기준 2026-08-01부터 true) 추가, `sync-merge` 잡 if 에 `sunset != 'true'` 전제 — u81-baseball 서비스 종료(7/31) 이후 데이터 동기화 자동 중단(잡은 회색 스킵). base 자체 수집·배포는 계속.
- 2026-07-11: **모바일 콜드 스타트 홈런 아크**: `web/public/launch-home-run.webm`(VP9, 512×512, 30fps, 1.03초, 무음, 42.4KB) + `mobile/MobileLaunchMotion.tsx`. 모바일 새 문서 로드(앱 kill 후 재실행 포함)마다 한 번 흰색 fixed 오버레이에서 중앙 `clamp(280px,72vw,320px)`로 재생하고 180ms 페이드 후 메인 노출. 모듈 메모리 `mobileLaunchCompleted`라 같은 문서의 라우트 이동·백그라운드 복귀에는 재생하지 않음. 재생 오류/2.2초 최대 대기 폴백, `prefers-reduced-motion`은 정적 icon-512를 240ms 표시. Android OS 스플래시 하단 앱 이름은 플랫폼 제어라 웹에서 제거 불가하며 인앱 오버레이에는 추가 문구 없음.
- 2026-07-11: **모바일 연속 캐러셀 보완**: 메인 시즌리더는 타자 6항목(`avg/hr/rbi/h/r/sb`) 뒤 투수 5항목(`era/whip/so/w/ip`)을 단일 11장 트랙으로 연결해 경계에서 양방향 스와이프 가능. 타자/투수 칩은 해당 그룹 첫 장으로 이동하며 현재 장에 맞춰 활성 상태가 바뀜. 공용 `PagedCardGrid`를 추가해 모바일 경기일정 학교는 10개(2열×5행), 선수현황 학교는 8개(2열×4행)씩 한 페이지로 표시하고 카드 세로 여백을 축소. 데스크탑에서는 기존 반응형 그리드 유지.
- 2026-07-12: **Base 데이터 즉시 갱신 3조건**: 주기 폴링 없이 ① 새 문서 로드(앱 완전 종료 후 실행), ② `visibilitychange` 백그라운드 복귀, ③ 사용자 수동 갱신(상단 버튼·모바일 최상단 pull-to-refresh)에서만 최신 `data/meta.json`을 `no-store`+시각 쿼리로 확인. `data.ts`의 모든 JSON URL에 문서 시작/`lastUpdated`/수동 고유 리비전 `?v=`를 붙여 GitHub Pages `max-age=600` HTTP 캐시를 우회한다. 자동 복귀 갱신은 기존 화면을 유지하고 실패해도 기존 데이터를 보존한다. 수동 갱신은 모래시계를 최소 450ms 표시한 뒤 문서를 다시 로드해 홈런 아크 시작 모션과 시작 리비전 갱신을 실행한다. `focus`/`online` 이벤트와 타이머는 사용하지 않는다.
- 2026-07-12: **설치 PWA 번들 갱신 보강**: 배포 후 서버에는 새 JS가 있으나 기존 서비스워커가 동일한 `v2`여서 설치본이 `max-age=600`의 이전 HTML·번들을 유지하는 현상을 실기기에서 확인. 서비스워커 캐시를 `u18-baseball-v3`로 올리고, 등록 시 `updateViaCache:'none'`+즉시 `registration.update()`를 사용한다. 기존 컨트롤러가 새 서비스워커로 교체되면 `controllerchange`에서 문서를 한 번 재로드하며, navigation 요청은 `cache:'reload'`로 최신 index와 해시 번들을 받는다.
- 2026-07-12: **1.15초 시작·새로고침 모션 분리**: 일반 앱 실행은 1번 `refresh-scoreboard-flip.webm` 스코어보드 플립 1.15초로 고정. 상단 버튼·pull-to-refresh 수동 갱신은 재로드 직전 `refreshMotion.ts`가 2번 패스트 피치 → 3번 홈런 아크 → 1번 스코어보드 플립 순으로 `sessionStorage`에 저장해 반복한다. 새 문서의 `MobileLaunchMotion`이 선택값을 한 번 소비·삭제해 1개만 재생하며, 저장값이 없거나 잘못되면 1번 스코어보드 플립으로 폴백한다. 앱 최초 실행은 이 순환에 영향을 주지 않고, 백그라운드 복귀 자동 갱신에는 모션을 재생하지 않는다.
- 2026-07-12: **모션 순백 배경 최종화**: 3종 WebM의 아이콘 외곽 옅은 둥근 사각형·회색 배경 흔적을 제거해 앱 오버레이와 동일한 `#fff`로 통일. 2번 패스트 피치는 공이 화면 왼쪽 밖에서 오른쪽 밖까지 통과하면서 지나간 영역의 `U18 BASEBALL PLAYER STATS` 로고를 왼쪽부터 순차 노출하고, 공은 화면 밖으로 빠진 뒤 완성 로고만 남는다. 1번 스코어보드 플립과 3번 홈런 아크 동작을 유지한다. 세 파일 모두 1.15초·512×512·60fps이며 URL 버전은 `20260712-clean-115`.
- 2026-07-11: **드로어 PNG 아이콘 사전 로드**: 첫 햄버거 클릭 때만 느린 원인은 조건부 드로어 마운트 시 `navIcons.tsx`의 PNG 8개가 최초 요청·디코딩되기 때문. `preloadNavIcons()`가 앱 시작(`main.tsx`)과 함께 `Image` 객체를 생성해 비동기 디코딩하고, 완료 전까지 `Set<HTMLImageElement>`로 객체를 유지. 기존 PNG 디자인·드로어 구조는 유지하며 첫 클릭 시 이미지 준비 비용을 제거.
- 2026-07-10: **메뉴·페이지 제목 아이콘 8종**: 사용자 제공 PNG(투명배경 256px 라인아트)를 `web/src/assets/icons/` 에 배치, `shared/navIcons.tsx` 의 `<Ico name=… variant="nav"|"title">` 로 렌더. 매핑 = calendar→경기일정 / profile→선수현황 / jersey(등번호 22)→선수 기록 / statistics→항목별 랭킹 / sports(교차 배트)→상대전적 / search→선수 검색 / chart→지표 설명 / announcement→알리는 글. 적용처는 드로어(데스크탑 nav-drawer + 모바일 m-drawer, `.nav-ico` 20/22px)와 각 페이지 제목 옆(`.title-ico` = `1em` 이라 heading 폰트 따라감 — 데스크탑 32px/모바일 26px). 데스크탑 가로 내비는 텍스트 유지(폭 여유 없음). **원본은 선이 얇아(256px 기준 ~3px) UI 크기에서 회색으로 뭉개짐** → 앱 사본만 알파 채널 `MaxFilter(5)` 팽창 + `#111` 채색 후 96px 로 리샘플해 저장(원본 경로 파일은 미변경). k=7 은 유니폼 "22"·달력 점이 붙어 k=5 채택. statistics.png 는 4KB 미만이라 Vite 가 base64 인라인.
- 2026-07-10: **홈 이동 아이콘 (홈베이스)**: `shared/HomeIcon.tsx` — 야구 홈베이스 오각형 인라인 SVG(`M4 4h16v9l-8 8-8-8z`, currentColor, 각진 모서리). 브랜드 로고 `U18 BASEBALL` 왼쪽에만 부착(데스크탑 상단바·모바일 상단바·모바일 드로어 상단 브랜드 3곳) — 드로어에 별도 "홈" 메뉴 항목은 두지 않음(기존 메뉴 구조 유지, 사용자 확정). `.brand` inline-flex + `.home-icon` 20px + 디스플레이 폰트 라인 보정(margin-top -2px). 집 글리프 U+2302 는 모바일 폰트 지원이 불안정해 SVG 채택.
