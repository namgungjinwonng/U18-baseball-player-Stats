...
- 2026-07-12: **Base 데이터 즉시 갱신 3조건**: 주기 폴링 없이 ① 새 문서 로드(앱 완전 종료 후 실행), ② `visibilitychange` 백그라운드 복귀, ③ 사용자 수동 갱신(상단 버튼·모바일 최상단 pull-to-refresh)에서만 최신 `data/meta.json`을 `no-store`+시각 쿼리로 확인. `data.ts`의 모든 JSON URL에 문서 시작/`lastUpdated`/수동 고유 리비전 `?v=`를 붙여 GitHub Pages `max-age=600` HTTP 캐시를 우회하고, 리비전 변경 시 마운트된 일정·선수현황·기록 훅을 일괄 재조회한다. 재조회 중 기존 데이터를 유지하고 실패해도 기존 화면을 보존한다. `focus`/`online` 이벤트와 타이머는 사용하지 않는다.
