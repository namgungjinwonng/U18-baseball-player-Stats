...
- 2026-07-12: **1.15초 시작·새로고침 모션 분리**: 일반 앱 실행은 `refresh-scoreboard-flip.webm` 스코어보드 플립 1.15초로 고정. 상단 버튼·pull-to-refresh 수동 갱신은 재로드 직전 `refreshMotion.ts`가 스코어보드 플립/패스트 피치/홈런 아크 1.15초 중 하나를 무작위로 `sessionStorage`에 저장한다. 직전 랜덤 모션은 후보에서 제외해 동일 모션이 연속 재생되지 않는다. 새 문서의 `MobileLaunchMotion`이 선택값을 한 번 소비·삭제해 1개만 재생하며, 저장값이 없거나 잘못되면 스코어보드 플립으로 폴백한다. 일반 앱 실행의 고정 스코어보드 플립은 랜덤 이력에 포함하지 않고, 백그라운드 복귀 자동 갱신에는 모션을 재생하지 않는다.
- 2026-07-12: **모션 순백 배경 최종화**: 3종 WebM의 아이콘 외곽 옅은 둥근 사각형·회색 배경 흔적을 제거해 앱 오버레이와 동일한 `#fff`로 통일. 2번 패스트 피치는 공이 화면 왼쪽 밖에서 오른쪽 밖까지 통과하면서 지나간 영역의 `U18 BASEBALL PLAYER STATS` 로고를 왼쪽부터 순차 노출하고, 공은 화면 밖으로 빠진 뒤 완성 로고만 남는다. 1·3번은 기존 홈런 아크/스코어보드 플립 동작을 유지한다. 세 파일 모두 1.15초·512×512·60fps이며 URL 버전은 `20260712-clean-115`.
